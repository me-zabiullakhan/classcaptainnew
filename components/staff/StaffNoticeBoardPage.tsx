...
export function StaffNoticeBoardPage({ onBack, notices }: { onBack: () => void; notices: Notice[] }) {
    
    const relevantNotices = useMemo(() => {
        const now = new Date();
        return notices
            .filter(n => (n.targetAudience === 'All' || n.targetAudience === 'Staff') && n.expiryDate.toDate() >= now)
